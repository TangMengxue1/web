		...
		<div class="contain">
			<div class="big-box" :class="{active:isLogin}">
				...
				<div class="big-contain" key="bigContainRegister" v-else>
					...
					<div class="bform">
						...
						<input type="email" placeholder="电话" v-model="form.userphone">
						<input type="password" placeholder="密码" v-model="form.userpwd">
					</div>
			...
			<div class="small-box" :class="{active:isLogin}">
				<div class="small-contain" key="smallContainRegister" v-if="isLogin">
					...
					<p class="scontent">开始注册，和我们一起探索非遗文化吧</p>
					<button class="sbutton" @click="changeType">注册</button>
				</div>
				<div class="small-contain" key="smallContainLogin" v-else>
					<div class="stitle">欢迎回来!</div>
					<p class="scontent">请登录你的账户，进入非遗世界吧</p>
		...
		methods:{
			...
			login() {
				...
				if (self.form.username != "" && self.form.userpwd != "") {
					self.$axios({
						method:'post',
						url: 'http://127.0.0.1:3380/api/user/login',
						data: {
			...
			register(){
				...
				if(self.form.username != "" && self.form.userphone != "" && self.form.userpwd != ""){
					self.$axios({
						method:'post',
						url: 'http://127.0.0.1:3380/api/user/add',
						data: {
							username: self.form.username,
